...
if supports_scc; then
    ...
    # Note: this script only works on OpenShift >= 3.11, otherwise you must run the following command manually
      oc adm policy add-scc-to-group ibm-sccm-scc system:serviceaccounts:$namespace
    else
...
if supports_psp; then
    ...
    sed 's/{{ NAMESPACE }}/'$namespace'/g' ibm-sccm-rb.yaml > $namespace-ibm-sccm-rb.yaml
    ...
    kubectl apply -f $namespace-ibm-sccm-rb.yaml -n $namespace
